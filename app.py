...
def build_analytics(entries, staff_type="picker"):
    try:
        ...
        return dict(pick_acc=pick_acc, pick_speed=pick_speed, check_acc=check_acc,
...
def admin_dashboard():
    try:
        ...
        for emp in employees:
            ents       = KPIEntry.query.filter_by(emp_id=emp.id).all()
        ...
        total_bills  = sum(e.sales_bills_open or 0 for e in all_ents)
        ...
        return render_template("admin.html", rows=[], total_picked=0,
                               total_bills=0, total_entries=0, active_today=0, emp_count=0)
...
def admin_edit_user(emp_id):
    """Edit employee details and optionally reset password."""
    try:
        emp = db.session.get(Employee, emp_id)
...
def admin_export_csv():
    """Export all KPI data as downloadable CSV."""
    try:
        import csv
        output = io.StringIO()
...
def api_stats(emp_id):
    """JSON endpoint for employee stats."""
